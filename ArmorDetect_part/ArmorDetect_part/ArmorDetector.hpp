...
struct Light : public cv::RotatedRect     //灯条结构体
{
    ...
    explicit Light(cv::RotatedRect& box) : cv::RotatedRect(box)
    {
        ...
        angle = (top.x <= bottom.x) ? (box.angle) : (90 + box.angle);//矫正angle
...
private:
    ...
    cv::Mat _src;  // 裁剪src后的ROI--region of insterest
    cv::Mat _binary;
    ...
    std::multimap<int, SpinTracker> trackers_map;  //定义map一个对象"tracker_map预测器"
    const int max_delta_t = 50;                //使用同一预测器的最大时间间隔(ms)
    const double max_delta_dist = 40;              // 最大追踪距离
    std::map<int, SpinHeading> spin_status_map;     // 记录该车小陀螺状态state（未知，顺时针，逆时针）
    //std::map<int, double> spin_score_map;           // 记录各装甲板小陀螺可能性分数，大于0为逆时针旋转，小于0为顺时针旋转
    std::unordered_map<int, double> spin_score_map;           // 哈希表实现的无序关联容器 降低时间复杂度
    ...
    bool isLight(Light& light, std::vector<cv::Point>& cnt);//根据light的信息 和 由contours得到的面积
